...
2. **Fallback decision**: If the primary fails with a capacity or availability failure, Bifrost moves to the first fallback. That covers errors with no HTTP status (network errors, timeouts, connection-pool exhaustion), `408`, `429`, any `5xx`, and requests Bifrost did not hand to the provider (no key serves the model, a governance refusal of that provider, or a Bifrost-side error). Any other `4xx` is the provider's verdict on the request itself, so Bifrost returns it immediately. The same rule decides whether a failed fallback moves on to the next one
...
5. **All fail**: Returns the error from the last provider tried. A fallback whose error ends the chain (a `4xx` rejection, or a plugin setting `AllowFallbacks = false`, e.g. a security or compliance plugin that should halt the chain regardless of remaining fallbacks) is returned immediately without trying the remaining providers.
...
| All fallbacks exhausted | Error | `All N fallback(s) exhausted; returning last error (<type> HTTP <code>)` |
...
**Plugin fallback control:** Plugins can override the fallback decision for their errors. For example, a security plugin might disable fallbacks for compliance reasons. When a plugin sets `AllowFallbacks = false` on the error, the fallback chain is skipped entirely and the original error is returned immediately. `AllowFallbacks = true` tries the fallbacks whatever the status, and leaving it unset applies the capacity and availability rule above.
