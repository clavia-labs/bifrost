locals {
  ...
  entrypoint       = local.teardown_enabled ? ["/bin/bash", "-c", file("${path.module}/entrypoint.sh"), "teardown"] : null
...
resource "terraform_data" "wait_for_ready" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      until curl -sf --connect-timeout 10 --max-time 30 -o /dev/null -H "Authorization: Bearer $VLLM_API_KEY" "$BASE_URL/v1/models"; do
